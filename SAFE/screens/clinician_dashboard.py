...
# PATIENT ROW
# ============================================================
def _render_patient_row(patient: dict):
    ...
    with col_actions:
        ...
        with a2:
            if st.button("✏️ Edit", key=f"edit_{pid}", use_container_width=True,
                ...
                st.rerun()
        with a3:
            if st.button("🩺 Update", key=f"update_{pid}", use_container_width=True,
                         help="Update vitals and labs"):
                st.session_state["modal_target"] = pid
                st.session_state["modal_kind"] = "update"
                st.rerun()
        with a4:
            if st.button("↗ Discharge", key=f"discharge_{pid}", use_container_width=True,
...
def _modal_update(patient_id: str):
    ...
    with tab_manual:
        ...
        with v1:
            hr = st.number_input("Heart rate (bpm)",   value=float(staged["vitals"]["hr"]),     key=f"v_hr_{patient_id}")
            sbp = st.number_input("SBP (mmHg)",         value=float(staged["vitals"]["sbp"]),    key=f"v_sbp_{patient_id}")
        with v2:
            map_ = st.number_input("MAP (mmHg)",        value=float(staged["vitals"]["map"]),    key=f"v_map_{patient_id}")
            temp = st.number_input("Temp (°C)",         value=float(staged["vitals"]["temp"]),   key=f"v_temp_{patient_id}", step=0.1, format="%.1f")
        with v3:
            resp = st.number_input("Resp (/min)",       value=float(staged["vitals"]["resp"]),   key=f"v_resp_{patient_id}")
            o2sat = st.number_input("O2 Sat (%)",       value=float(staged["vitals"]["o2sat"]),  key=f"v_o2_{patient_id}")

        st.markdown("##### Key labs")
        l1, l2, l3 = st.columns(3)
        with l1:
            lactate    = st.number_input("Lactate (mmol/L)",     value=float(staged["labs"]["lactate"]),    key=f"l_lac_{patient_id}", step=0.1, format="%.1f")
            platelets  = st.number_input("Platelets (x10⁹/L)",   value=float(staged["labs"]["platelets"]),  key=f"l_plt_{patient_id}")
        with l2:
            wbc        = st.number_input("WBC (x10⁹/L)",         value=float(staged["labs"]["wbc"]),        key=f"l_wbc_{patient_id}", step=0.1, format="%.1f")
            bun        = st.number_input("BUN (mg/dL)",          value=float(staged["labs"]["bun"]),        key=f"l_bun_{patient_id}")
        with l3:
            creatinine = st.number_input("Creatinine (mg/dL)",   value=float(staged["labs"]["creatinine"]), key=f"l_cr_{patient_id}",  step=0.1, format="%.1f")
            glucose    = st.number_input("Glucose (mg/dL)",      value=float(staged["labs"]["glucose"]),    key=f"l_glu_{patient_id}")
